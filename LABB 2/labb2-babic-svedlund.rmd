...
har ett effekt på regnmängd är 
parat t-test. Om skillnaderna i nederbörd mellan dagarna med och utan molnsådd är 
normalfördelade, kan ett parat t-test användas. Detta test jämför de 
genomsnittliga skillnaderna i nederbörd mellan de två dagarna i varje period.
...
```{r}
t_test <- t.test(seed, nonseed, alternative = "two.sided", mu = 0, paired = TRUE, conf.level = 0.95)
print(t_test)
```

Vi använder alltså t-testet för två parvist beroende stickprov med felrisk $\alpha = 0.05$. Vi förkastar nollhypotesen om 
p-värdet är större än felrisken. Vi ser här att enligt beräkningen ovan så är $0.3024=$`p-value`$>\alpha = 0.05$. Vi 
